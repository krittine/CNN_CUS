...
CLASS zcl_apj_obj_gen IMPLEMENTATION.
  ...
  METHOD create_job.
    ...
    lv_template_name = 'ZBJ_DO_TMP'.
    lv_jobname       = |APP_JOB_DO_| && iv_do && |_| && cl_abap_context_info=>get_system_date( ) && cl_abap_context_info=>get_system_time( ).
*    lv_job_text      = |Application Job DO | && iv_do.
    lv_job_text      = 'Update serial in Outbound Delivery'.
